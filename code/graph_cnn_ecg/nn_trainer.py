...
class end_to_end_trainer:
    ...
    def get_output(self, instance, d, L, lmax, perm, ep):
        return self.nn_model(instance, d, L, lmax, perm, ep)

    def proc_inp(self, inp):
        return inp

    def proc_pred(self, pred):
        return pred

    def proc_pred_lab(self, plab):
        return plab

    def train_model(self, d=None, L=None, lmax=None, perm=None, num_epoch=15):
        # pdb.set_trace()
        print('TrainSet: ', len(self.train_loader))
        ...
        for epoch in range(num_epoch):
            ...
            for ind, sample in enumerate(tqdm(self.train_loader)):
                ...
                # pdb.set_trace()
                self.optimizer.zero_grad()
                y_pred = self.get_output(instance, d, L, lmax, perm,
                                         self.start_epoch + epoch)
                y_pred = y_pred.view(-1, 2)
                y_pred = self.proc_pred(y_pred)
                # pdb.set_trace()
                loss = self.loss_fn(y_pred, label)
                loss.backward()
                self.optimizer.step()
    ...
    def test_model(self, d=None, L=None, lmax=None, perm=None):
        print('TestSet :', len(self.test_loader))
        num_corr = 0
        tot_num = 0
        false_0 = 0
        ...
        for sample in tqdm(self.test_loader):
            ...
            y_pred = y_pred.view(-1, 2)
            _, label_pred = torch.max(y_pred.data, 1)
            label_pred = self.proc_pred_lab(label_pred)
...
class ml_cnn_trainer(end_to_end_trainer):
    ...
    def get_output(self, instance, d, L, lmax, perm, ep):
        # pdb.set_trace()
        return self.nn_model(instance)


class pe2e_trainer(ml_cnn_trainer):
    def get_output(self, instance, d, L, lmax, perm, ep):
        return self.nn_model(instance, L[0])


class ml_cnn_consensus_trainer(ml_cnn_trainer):
    def proc_inp(self, inp):
        self.B, self.cons, Din, num_c = inp.shape
        return inp.view(self.B*self.cons, Din, num_c)

    def proc_pred(self, pred):
        return pred.view(self.B, self.cons, 2)

    def proc_pred_lab(self, plab):
        label_pred_cons = plab.view(self.B, self.cons)
        label_pred, _ = label_pred_cons.mode(dim=1)
        return label_pred
    # def train_model(self, d=None, L=None, lmax=None, perm=None, num_epoch=15):
    #     print('TrainSet: ', len(self.train_loader))
    #     self.tot_ind = len(self.train_loader)
    ...
    #         for ind, sample in enumerate(tqdm(self.train_loader)):
    #             instance = Variable(sample['sig'].cuda())
    #             # Instance is of shape : B x cons x Din x channels
    #             B, cons, Din, num_c = instance.shape
    #             instance = instance.view(B*cons, Din, num_c)
    #             label = Variable(sample['label'].type(dtypeLong))

    #             self.optimizer.zero_grad()
    #             y_pred = self.get_output(instance, d, L, lmax, perm,
    #                                      self.start_epoch + epoch)
    #             assert y_pred.shape == (B*cons, 2)
    #             y_pred = y_pred.view(B, cons, 2)
    #             # pdb.set_trace()
    #             loss = self.loss_fn(y_pred, label)
    #             loss.backward()
    #             self.optimizer.step()
    ...
    #         y_pred = self.get_output(instance, d, L, lmax, perm, self.curr_epoch)
    #         y_pred = y_pred.view(-1, 2)
    #         _, label_pred_cons = torch.max(y_pred.data, 1)
    #         label_pred_cons = label_pred_cons.view(B, cons)
    #         label_pred = label_pred_cons.mode(dim=1)
